...
// Calculate node-specific loss gradients
float* calculate_loss_gradients(float** node_features, int num_nodes) {
    float* loss_grads = (float*)malloc(num_nodes * sizeof(float));
    
    for (int i = 0; i < num_nodes; i++) {
        // Calculate prediction from the first feature (can be modified based on your task)
        float pred = sigmoid(node_features[i][0]);
        // Binary target (can be modified based on your specific task)
        float target = (i == 0) ? 1.0f : 0.0f;
        
        // Gradient of binary cross-entropy with respect to prediction
        loss_grads[i] = (pred - target) / (pred * (1.0f - pred) + 1e-9f);
    }
    
    return loss_grads;
}

// Modified backward pass: Compute gradients with loss propagation
void backward_pass(const Node* graph, float** node_features, const GNNParameters* params, 
                   GNNParameters* gradients, float** cache, float* loss_grads) {
    ...
    // Create feature gradients to store intermediate gradients
    float** feature_gradients = (float**)malloc(NUM_NODES * sizeof(float*));
    for (int i = 0; i < NUM_NODES; i++) {
        feature_gradients[i] = (float*)calloc(NUM_FEATURES, sizeof(float));
        
        // Initialize with the loss gradients
        for (int k = 0; k < NUM_FEATURES; k++) {
            // Apply loss gradients proportionally to each feature
            // For simplicity, we'll focus the gradient on the first feature
            feature_gradients[i][k] = (k == 0) ? loss_grads[i] : loss_grads[i] * 0.1f;
        }
    }

    // Backpropagate through layer normalization and ReLU
    for (int i = 0; i < NUM_NODES; i++) {
        // ReLU gradient: Only propagate if the feature was > 0
        for (int k = 0; k < NUM_FEATURES; k++) {
            if (node_features[i][k] <= 0) {
                feature_gradients[i][k] = 0.0f;
            }
        }
        
        // Approximate layer norm gradient (simplified)
        float sum_grad = 0.0f;
        for (int k = 0; k < NUM_FEATURES; k++) {
            sum_grad += feature_gradients[i][k];
        }
        float mean_grad = sum_grad / NUM_FEATURES;
        
        for (int k = 0; k < NUM_FEATURES; k++) {
            feature_gradients[i][k] -= mean_grad;
        }
    }

    // Compute gradients for attention mechanism and weights
    for (int i = 0; i < NUM_NODES; i++) {
        if (graph[i].num_neighbors > 0) {
            float* attention_scores = cache[i];
            ...
            for (int k = 0; k < NUM_FEATURES; k++) {
                for (int l = 0; l < NUM_FEATURES; l++) {
                    ...
                    for (int j = 0; j < graph[i].num_neighbors; j++) {
                        int neighbor = graph[i].neighbors[j];
                        grad_sum += attention_scores[j] * node_features[neighbor][l] * feature_gradients[i][k];
                    }
                    gradients->weights[k][l] += grad_sum;
                }
            }
            
            // Calculate attention score gradients
            float* attention_score_grads = (float*)calloc(graph[i].num_neighbors, sizeof(float));
            for (int j = 0; j < graph[i].num_neighbors; j++) {
                int neighbor = graph[i].neighbors[j];
                for (int k = 0; k < NUM_FEATURES; k++) {
                    attention_score_grads[j] += feature_gradients[i][k] * node_features[neighbor][k];
                }
            }
            
            // Apply softmax gradient
            float sum_attention_grads = 0.0f;
            for (int j = 0; j < graph[i].num_neighbors; j++) {
                sum_attention_grads += attention_score_grads[j] * attention_scores[j];
            }
            
            for (int j = 0; j < graph[i].num_neighbors; j++) {
                attention_score_grads[j] = attention_scores[j] * (attention_score_grads[j] - sum_attention_grads);
            }
            
            // Gradients for attention weights and bias
            for (int j = 0; j < graph[i].num_neighbors; j++) {
                int neighbor = graph[i].neighbors[j];
                float score_grad = attention_score_grads[j] / sqrtf(NUM_FEATURES);
            ...
            free(attention_score_grads);
        }
    }
    
    // Clean up feature gradients
    for (int i = 0; i < NUM_NODES; i++) {
        free(feature_gradients[i]);
    }
    free(feature_gradients);
}
...
int main() {
    ...
    for (int test = 0; test < num_tests; test++) {
        ...
        for (int epoch = 0; epoch < NUM_EPOCHS; epoch++) {
            ...
            float* loss_grads = calculate_loss_gradients(node_features, NUM_NODES);
            backward_pass(graph, node_features, &params, &gradients, cache,loss_grads);
